...
from amazon import AmazonPhoneSearch
app = Flask(__name__)
api = Api(app)
...
class Order(Resource):
    def get(self):
        ...
        if not(type(start_price)==float and type(end_price)==float and type(product_name)==str and type(product_company)==str):
        ...
        try:
            print(self.username,self.password,self.driver_path,start_price,end_price,product_name,product_company)
            order = AmazonPhoneSearch(self.username,self.password,self.driver_path,start_price,end_price,product_name,product_company)
            order.run()
